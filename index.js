...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (interaction.customId.startsWith('moffer_')) {
            ...
            if (!seller) {
                return interaction.reply({
                    content: 'Der Verkäufer konnte nicht gefunden werden.',
                    ephemeral: true
                });
            }
            ...
            try {
                ...
                return interaction.reply({
                    content: 'Dein Angebot wurde dem Verkäufer gesendet.',
                    ephemeral: true
                });
            } catch (error) {
                console.error('Fehler beim Senden des Angebots:', error.message);
                return interaction.reply({
                    content: 'Ich konnte dem Verkäufer keine DM schicken. Wahrscheinlich sind seine DMs deaktiviert.',
                    ephemeral: true
                });
            }
        }
    ...
    if (interaction.isButton()) {
        ...
        if (action === 'sold') {
            if (interaction.user.id !== sellerId) {
                return interaction.reply({ content: 'Nur der Verkäufer kann das!', ephemeral: true });
            }
            ...
            return interaction.reply({
                content: 'Item wurde gelöscht und aus Favoriten entfernt.',
                ephemeral: true
            });
        }
...
client.on('messageCreate', async message => {
    if (message.author.bot || !activeUploads.has(message.author.id)) {
        return;
    }
